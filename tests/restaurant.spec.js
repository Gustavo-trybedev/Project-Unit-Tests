...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    expect(createMenu({ food: { coxinha: 4, sanduiche: 6 }, drink: { coca: 5, agua: 3 }, }).fetchMenu()).toEqual({
      food: { coxinha: 4, sanduiche: 6 },
      drink: { coca: 5, agua: 3 },
    });
    ...
    expect(createMenu({ food: { coxinha: 4, sanduiche: 6 }, drink: { coca: 5, agua: 3 }, }).consumption).toEqual([]);
    ...
    const returnedObj = createMenu({ food: { coxinha: 4, sanduiche: 6 }, drink: { coca: 5, agua: 3 }, });
    returnedObj.order('coxinha');
    expect(returnedObj.consumption).toContain('coxinha');
    ...
    const returnedObj2 = createMenu({ food: { coxinha: 4, sanduiche: 6 }, drink: { coca: 5, agua: 3 }, });
    returnedObj2.order('coxinha');
    returnedObj2.order('agua');
    returnedObj2.order('sopa');
    returnedObj2.order('sashimi');
    expect(returnedObj2.consumption).toEqual(['coxinha', 'agua', 'sopa', 'sashimi']);
    ...
    const returnedObj3 = createMenu({ food: { coxinha: 4, sanduiche: 6 }, drink: { coca: 5, agua: 3 }, });
    returnedObj3.order('coxinha');
    returnedObj3.order('agua');
    returnedObj3.order('coxinha');
    expect(returnedObj3.consumption).toEqual(['coxinha', 'agua', 'coxinha']);
    ...
    const priceObj = createMenu({ food: { coxinha: 4, sanduiche: 6 }, drink: { coca: 5, agua: 3 }, });
    priceObj.order('coxinha');
    priceObj.order('agua');
    priceObj.order('coxinha');
    expect(priceObj.pay()).toBe(11);
